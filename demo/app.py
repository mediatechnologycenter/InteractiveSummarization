...
from dataset.guidance.sents import guidance_sent_extract


parser = argparse.ArgumentParser("Guided Summarization Demo")
parser.add_argument("model_path", type=str)
args = parser.parse_args()
pipe = GuidedSummarizationPipeline(args.model_path)


def predict(article, reference, guidance, min_length, max_length, length_penalty):
    if not guidance and not reference:
        return ("Please provide either reference or guidance.", "")
    guidance = guidance if guidance else guidance_sent_extract(article, reference)
    predictions = pipe(
        {"article": article, "guidance": guidance},
        min_length=min_length,
        max_length=max_length,
        length_penalty=length_penalty,
        clean_up_tokenization_spaces=True,
    )
    prediction = predictions[0]["summary_text"].replace("\n", " ")
    return (prediction, guidance)


# Inputs

article_box = gr.inputs.Textbox(
    label="Article",
    placeholder="Article to summarize",
    lines=20,
)

reference_box = gr.inputs.Textbox(
    label="Reference",
    placeholder="Provide the reference for automatic guidance (oracle)",
    lines=3,
)

guidance_box = gr.inputs.Textbox(
    label="Guidance",
    placeholder="Provide you own guidance signal ('reference' will be "
    "ignored). Each sentence should be separated by '<q>'.",
    lines=3,
)

min_length_deafult = 55
min_gen_length_box = gr.inputs.Slider(
    minimum=10,
    maximum=100,
    step=5,
    default=min_length_deafult,
    label="Minimum generation length",
)

max_length_deafult = 140
max_gen_length_box = gr.inputs.Slider(
    minimum=50,
    maximum=200,
    default=max_length_deafult,
    step=5,
    label="Maximum generation length",
)

length_penalty_default = 2
length_penalty_box = gr.inputs.Slider(
    minimum=1, maximum=5, step=1, default=length_penalty_default, label="Length penalty"
)

# Outputs

summary_box = gr.outputs.Textbox(label="Summary")

guidance_used_box = gr.outputs.Textbox(label="Used guidance signal")
...
examples = [
    i + [min_length_deafult, max_length_deafult, length_penalty_default]
    for i in examples
]
...
interface = gr.Interface(
    ...
    outputs=[summary_box, guidance_used_box],
    examples=examples,
    css="""
    .title {
        color:black;
    ...
    .title:before {
        content: "";
        display: block;
        background: url("https://ethz.ch/etc/designs/ethz/img/header/ethz_logo_black.svg") no-repeat;
        width: 160px;
        height: 45px;
        float: left;
        margin: 0 6px 0 0;
    ...
    .title:after {
        content: "";
        display: block;
        background: url("https://mtc.ethz.ch/_jcr_content/orgbox/image.imageformat.logo.369189574.svg") no-repeat;
        width: 160px;
        height: 45px;
        float: right;
        margin: 0 6px 0 0;
