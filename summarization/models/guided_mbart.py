...
from transformers import MBartModel, MBartConfig, MBartForConditionalGeneration
...
class GuidedMBartDecoderLayer(nn.Module):
    def __init__(self, config: MBartConfig):
        ...
        self.encoder_attn1 = MBartAttention(
            self.embed_dim,
        ...
        self.encoder_attn2 = MBartAttention(
            self.embed_dim,
            config.decoder_attention_heads,
            dropout=config.attention_dropout,
            is_decoder=True,
    ...
    def forward(
        ...
        # NOTE: DIFF FROM BART (other location)
...
class GuidedMBartEncoder(MBartEncoder):
    def __init__(
        ...
        if self.config.source_top_encoder_layer:
            self.source_layer = MBartEncoderLayer(config)
        self.guidance_layer = MBartEncoderLayer(config)

        self.layer_norm_enc_1 = nn.LayerNorm(config.d_model)
        self.layer_norm_enc_2 = nn.LayerNorm(config.d_model)
    ...
    def forward(
        ...
        hidden_states = (
            self.layer_norm_enc_1(hidden_states[0]),
            self.layer_norm_enc_2(hidden_states[1]),
        )
...
class GuidedMBartDecoder(MBartDecoder):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [GuidedMBartDecoderLayer(config) for _ in range(config.decoder_layers)]
        )
...
class GuidedMBartModel(MBartModel):
    def __init__(self, config: MBartConfig):
        ...
        self.encoder = GuidedMBartEncoder(config, self.shared)
        self.decoder = GuidedMBartDecoder(config, self.shared)
...
class GuidedMBartConfig(MBartConfig):
    model_type = "guided_mbart"
...
class GuidedMBartForConditionalGeneration(
    GuidedModelLoadMixin, MBartForConditionalGeneration
):
    config_class = GuidedMBartConfig

    def __init__(self, config: MBartConfig):
        super().__init__(config)

        self.model = GuidedMBartModel(config)
